...
that model scale, not clinical fine-tuning, drives quality: MedGemma
1.5 4B does not improve over its same-size general-purpose sibling
...
\begin{itemize}
  ...
  \item \textbf{GPT-4.1-nano} (closed-source reference):
        ...
        API. We include it solely as an external reference to contextualize
        the open models, \emph{not} as a locally-deployable peer; it is
        therefore reported separately and excluded from ``best open
        model'' comparisons. Because closed APIs are versioned and may
        change without notice and their seed support is best-effort, we
        make no determinism claims for this model and run it only under
        the same decoding settings as the open models.
\end{itemize}
...
\centering
\caption{{\bf Quality metrics: model output vs.\ gold answer, mean [95\% CI].}
Per dataset, $n{=}50$ questions $\times$ 10 runs. Each value is the
across-question mean with a clustered percentile-bootstrap 95\% confidence
interval (1{,}000 resamples, resampling questions). Bold = best among the
open-weight models per column, per dataset.}
\label{tab:quality}
\footnotesize
\begin{tabular}{lcccc}
\toprule
...
\end{tabular}
\\[2pt]
{\footnotesize $^\dagger$ Closed-source API model (OpenAI), included as an
external reference point, not as a peer of the locally-deployable open-weight
models; excluded from the per-column ``best'' comparison.}
\end{adjustwidth}
\end{table}
...
\begin{adjustwidth}{-2.25in}{0in}
\centering
\caption{{\bf Within-model reproducibility across 10 runs, mean [95\% CI].}
Self-agreement (higher is better; $1/N{=}0.10$ = every run distinct) and
uniqueness (lower is better) are lexical; semantic self-similarity is the mean
pairwise BERTScore-F1 across the 10 runs (higher is better, robust to
paraphrase). Bold = best among the open-weight models per column, per dataset.}
\label{tab:repro}
\footnotesize
\begin{tabular}{lccc}
\toprule
...
\end{tabular}
\\[2pt]
{\footnotesize $^\dagger$ Closed-source API reference (see
Table~\ref{tab:quality}); excluded from the per-column ``best'' comparison.}
\end{adjustwidth}
\end{table}

\begin{table}[!ht]
\centering
\caption{{\bf Inference efficiency (MedQuAD, tokens/second).} Median per-run
throughput; MedicationQA throughput is comparable. GPT-4.1-nano is API-served
...
\caption{{\bf Within-model reproducibility by model, with 95\% bootstrap
confidence intervals.} Per-model lexical self-agreement ($\uparrow$), lexical
uniqueness ($\downarrow$), and semantic self-similarity ($\uparrow$) across the
10 runs. \textbf{(A)} MedQuAD (top); \textbf{(B)} MedicationQA (bottom). Across
both datasets and all five models, lexical agreement is low while semantic
...
\caption{{\bf Model-vs-gold quality by model, with 95\% bootstrap confidence
intervals.} BERTScore F1, Token F1, ROUGE-L, and LLM-judge score per model.
\textbf{(A)} MedQuAD (top); \textbf{(B)} MedicationQA (bottom). BERTScore is
tightly clustered across models; the closed reference GPT-4.1-nano leads on the
judge score but remains within the open models' range on the other metrics.}
\label{fig:quality}
\end{figure}
...
equivalent but differently worded answers count as non-overlapping. We
therefore frame these results as run-to-run \emph{output variability},
not as direct evidence of clinical inconsistency.
...
0.640 vs.\ 0.680 on MedicationQA, with overlapping confidence
intervals), indicating that at these scales capacity, not domain
fine-tuning, drives the differences (see Discussion).
...
First, \emph{scale helps}: Gemma 3 12B outperforms Gemma 3 4B on judge
score and is more reproducible on both datasets. Second, at fixed size,
\emph{clinical fine-tuning does not}: MedGemma 1.5 4B matches or trails
its same-size base model Gemma 3 4B on the judge score (0.452 vs.\ 0.460
on MedQuAD; 0.640 vs.\ 0.680 on MedicationQA) and shows no reproducibility
advantage, with confidence intervals that overlap throughout. The earlier
apparent ``MedGemma underperforms'' effect was therefore largely a
capacity effect: once size is held constant, domain adaptation confers no
measurable benefit on these consumer-health QA tasks.
...
limitation. For production evaluations, we recommend running the judge
multiple times per response and averaging, or adopting a deterministic
...
while the larger Gemma 3 12B leads the open models. The earlier apparent
shortfall of the clinically fine-tuned model was largely a capacity
effect. Model selection should be empirically driven, not label-driven.
...
\paragraph*{S1 File.}
\label{S1_File}
{\bf Sampled evaluation question sets.} The exact 50 questions used for
evaluation from each dataset (MedQuAD and MedicationQA), regenerable with
\texttt{clinical-eval export-sample -{}-sample-random 50 -{}-sample-seed 42}
(adding \texttt{-{}-prompt-file data/medicationqa.csv} for MedicationQA).
